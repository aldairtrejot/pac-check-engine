<template>
  <div>
    <tableTittleSearch value="Instancias" />
          ...
          <tr v-for="row in item" :key="row.id">
            <td class="text-center">
              <div class="button-container">
                <tableButtonEdit
                  :href="`${BASE_URL}/instancia/edit/` + row.id"
                  icon="fa fa-edit"
                  label="Editar"
                  bgColor="#10312B"
                />
            ...
            <td class="align-middle text-start">
              <span class="text-secondary text-xs font-weight-bold">{{ row.instancia }}</span>
            </td>
            <td class="align-middle text-center">
              <span class="text-secondary text-xs font-weight-bold">{{ row.anio }}</span>
            </td>
            <td class="align-middle text-center">
...
const fetchTableData = async () => {
  ...
  try {
    const { data } = await axios.post('/instancia/table', {
      limit: limit.value,
      offset,
